...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        cls_ctx_per_id = None
        if len(inputs) == 1:
            inputs = inputs[0] + self.attention(self.ln_1(inputs[0]))
                ...
                if len(compound_prompts_deeper) > 0:
                    if not self.text_layer:
                        if not (counter > len(compound_prompts_deeper) - 1):
                            ...
                            # visual_context_per_id = cls_ctx_per_id[counter].permute(1, 0, 2).half()
                        ...
                        if not (counter > len(compound_prompts_deeper) - 1):
                            ...
                            x = torch.cat([prefix, textual_context, cls_ctx_per_id_context, suffix], dim=0)
                            counter += 1
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            if len(inputs) == 3:
                ...
                return [x, compound_prompts_deeper, cls_ctx_per_id, counter]
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int, design_details):
        super().__init__()
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb=None, shared_ctx=None, shared_ctx_per_id=None, compound_deeper_prompts=None, visual_deep_prompts_per_id=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        if shared_ctx == None and compound_deeper_prompts == None:
            x = x.permute(1, 0, 2)  # NLD -> LND
            in_trans = [x]
        else:
            if self.VPT_shallow:
            ...
            x = x.permute(1, 0, 2)  # NLD -> LND;
            in_trans = [x, compound_deeper_prompts, 0]

        x11 = self.transformer.resblocks[:11](in_trans)
        x12 = self.transformer.resblocks[11]([x11[0]])
        x11 = x11[0].permute(1, 0, 2)  # LND -> NLD 
        x12 = x12[0].permute(1, 0, 2)  # LND -> NLD
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int, design_details):
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution, design_details
    )
    if vit:
