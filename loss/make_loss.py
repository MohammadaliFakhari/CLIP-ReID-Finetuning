...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if sampler == 'softmax':
        ...
        def loss_func(score, feat, target, target_cam, i2tscore = None, weights = None):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    if isinstance(score, list):
                        if weights != None:
                            ID_LOSS = [xent(scor, target, weights) for scor in score[0:]]
                            ID_LOSS = sum(ID_LOSS)
                        else:
                        ...
                        if weights != None:
                            ID_LOSS = xent(score, target, weights)
                        else:
                    ...
                    if i2tscore != None:
                        I2TLOSS = xent(i2tscore, target, weights)
